...
function presendscript {
  ...
  if [[ "$type_run_sssc" != "console" ]]; then
    ...
    if [[ "$err_status_sendfunc" -ne "1" ]]; then
      ...
      echo -e "\n${YELLOW}Примечание: Вводимое ниже значение определяет необходимость ожидания перезагрузки (параметр учитывается, если в списке для выполнения, после указанного скрипта, будут еще скрипты). Команда перезагрузки должна находиться в вашем скрипте (рекомендуется отложенная перезагрузка через shutdown -r +1 или (systemd-run bash -c \"sleep 15; reboot;\") для указания ожидания в секундах через sleep, т.к. если вы перезагрузите моментально, например через reboot, ssh сессия завершится принудительно и будет возвращен код ошибки, а также не удалятся отправленные файлы)$NoColor"
...
function changescriptfile {
  ...
  unset templist_selectvalue
  unset tempvalue
  unset list_block_changes_values
  unset title_msg
  unset text_msg
  unset name_variable
  unset temp_block_changes_values
  unset value_for_comparison
  unset status_check_value
  ...
  if [[ "${#listparamchange[@]}" -gt "0" ]]; then
    ...
    while [[ "$repeatcyclemain" -eq "1" ]]; do
      ...
      #Массив в который будут добавлены имена секций, для которых не будет задан вопрос об изменении значения переменной.
      list_block_changes_values=()
      ...
      for ((i = 0; i < ${#listparamchange[@]}; i++)); do
        ...
        #Продолжаем, если значение соответствует условию
        if [[ "${listparamchange[i]}" =~ $check_name_value ]]; then

          echo -e "\nОбработка секции ${GREEN}${listparamchange[i]}$NoColor"

          #Продолжаем, если указанная переменная найдена в скрипте
          if [[ "$(cat "$tfs/$tempnamescript" | grep "${listparamchange[i]}=" | wc -l)" -gt "0" ]]; then

            #Если массив list_block_changes_values не пуст и имя текущей секции присутствует в массиве list_block_changes_values, то переходим к следующей секции в цикле
            if [[ "${#list_block_changes_values[@]}" -gt "0" ]] && [[ "$(printf '%s\n' "${list_block_changes_values[@]}" | grep "^${listparamchange[i]}$" | wc -l)" -gt "0" ]]; then
              echo -e "\n${YELLOW}Изменение переменной ${listparamchange[i]} пропущено$NoColor"
              continue
            fi

            #Присваиваем имя следующей секции для использования в фильтре далее
            ...
            #Выполняется, если в указанном диапозоне строк (теккущей секции) есть terms_block_changes_curent_variable
            if [[ "$(sed -nr "/^\[${listparamchange[i]}\]/,/^\[$nextnamevalue\]/p" "$dir_scripts/$namesendscript/script.conf" | grep '^terms_block_changes_curent_variable=' | wc -l)" -eq "1" ]]; then

              echo -e "\n${GREEN}Проверка параметра terms_block_changes_curent_variable$NoColor"

              unset temp_terms_block_changes_curent_variable
              unset value_for_comparison
              temp_list_terms_block_changes_curent_variable=()

              #Записываем значение строки в массив по разделителю |
              readarray -d '|' -t temp_terms_block_changes_curent_variable < <(sed -nr "/^\[${listparamchange[i]}\]/,/^\[$nextnamevalue\]/p" "$dir_scripts/$namesendscript/script.conf" | sed -nr "{ :l /^terms_block_changes_curent_variable[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" | tr -d '\n')

              #Перебираем значения массива
              for ((tbcv = 0; tbcv < ${#temp_terms_block_changes_curent_variable[@]}; tbcv++)); do

                #Если в значении присутствует разделитель :, то продолжаем выполнение
                if [[ "$(echo "${temp_terms_block_changes_curent_variable[$tbcv]}" | grep -o ':' | wc -l)" -eq "1" ]]; then

                  #Получаем значение для сравнения из строки параметра
                  value_for_comparison="$(echo "${temp_terms_block_changes_curent_variable[$tbcv]}" | cut -d ':' -f1)"

                  #Записываем в массив значения (имена переменных) из второй части строки (после :) по разделителю ;.
                  readarray -d ';' -t temp_list_terms_block_changes_curent_variable < <(echo "${temp_terms_block_changes_curent_variable[$tbcv]}" | cut -d ':' -f2 | tr -d '\n')

                  #Продолжаем, если массив не пуст
                  if [[ "${#temp_list_terms_block_changes_curent_variable[@]}" -gt "0" ]]; then

                    #Перебираем значения массива
                    for ((tlbcv = 0; tlbcv < ${#temp_list_terms_block_changes_curent_variable[@]}; tlbcv++)); do

                      #Продолжаем, если указанная переменная найдена в файле скрипта
                      if [[ "$(cat "$tfs/$tempnamescript" | grep "${temp_list_terms_block_changes_curent_variable[$tlbcv]}=" | wc -l)" -gt "0" ]]; then

                        if [[ "$value_for_comparison" == "not_null" ]]; then

                          #Если value_for_comparison равно not_null, то проверяем, что значение в скрипте не пустое
                          if [[ -z "$(sed -nr "{ :l /${temp_list_terms_block_changes_curent_variable[tlbcv]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" ]] || [[ "$(sed -nr "{ :l /${temp_list_terms_block_changes_curent_variable[tlbcv]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" = "''" ]] || [[ "$(sed -nr "{ :l /${temp_list_terms_block_changes_curent_variable[tlbcv]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" = "\"\"" ]] || [[ "$(sed -nr "{ :l /${temp_list_terms_block_changes_curent_variable[tlbcv]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" = "()" ]]; then

                            #Если значение пустое, то переход к выполнению внешнего цикла temp_terms_block_changes_curent_variable
                            continue 2
                          fi
                        else
                          if ! [[ "$value_for_comparison" == "$(sed -nr "{ :l /${temp_list_terms_block_changes_curent_variable[tlbcv]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" ]]; then

                            #Если значения не равны, то переход к выполнению внешнего цикла temp_terms_block_changes_curent_variable
                            continue 2
                          fi
                        fi
                      else
                        echo -e "\n${RED}Не найдена переменная ${temp_list_terms_block_changes_curent_variable[$tlbcv]} в файле скрипта$NoColor"

                        #Если переменная не найдена, то переход к выполнению внешнего цикла temp_terms_block_changes_curent_variable
                        continue 2
                      fi
                    done

                    echo -e "\n${GREEN}Проверяемое значение/условие № $tbcv параметра terms_block_changes_curent_variable соответствует значению в файле скрипта.$NoColor"

                    echo -e "\n${YELLOW}Изменение переменной ${listparamchange[i]} пропущено.$NoColor"

                    #Если цикл temp_list_terms_block_changes_curent_variable был завершен, т.е. все значения проверены и не произошло выполнение continue 2 по определенным условиям, то пропускаем изменение текущей переменной (переход к выполнению внешнего цикла listparamchange)
                    continue 2
                  fi
                else
                  echo -e "\n${RED}Значение № $tbcv параметра terms_block_changes_curent_variable не соответствует формату$NoColor"
                fi
              done
            fi
              ...
              if [[ -n "$tempparamdesc" ]]; then
                ...
                if [[ "$ynaction" = "yes" ]]; then
                  ...
                  while [[ "$repeatcycle" -eq "1" ]]; do
                    ...
                    echo ""
                    echo "Новое значение переменной ${listparamchange[i]} в файле скрита: $(sed -nr "{ :l /${listparamchange[i]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")"
                ...
                #Выполняется, если в указанном диапозоне строк есть block_changes_values
                if [[ "$(sed -nr "/^\[${listparamchange[i]}\]/,/^\[$nextnamevalue\]/p" "$dir_scripts/$namesendscript/script.conf" | grep '^block_changes_values=' | wc -l)" -eq "1" ]]; then

                  echo -e "\n${GREEN}Проверка параметра block_changes_values$NoColor"

                  #Вывод сообщения и переопределение значения переменной
                  function check_value_script_and_conf {
                    echo -e "\n${GREEN}Проверяемое значение/условие № $tbcv параметра block_changes_values соответствует значению в файле скрипта.
Проверка и добавление указанных в параметре переменных в массив для пропуска запроса.$NoColor"
                    let status_check_value=1
                  }

                  unset temp_block_changes_values
                  unset value_for_comparison
                  temp_list_block_changes_values=()

                  #Записываем значение строки в массив по разделителю |
                  readarray -d '|' -t temp_block_changes_values < <(sed -nr "/^\[${listparamchange[i]}\]/,/^\[$nextnamevalue\]/p" "$dir_scripts/$namesendscript/script.conf" | sed -nr "{ :l /^block_changes_values[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" | tr -d '\n')

                  #Перебираем значения массива
                  for ((tbcv = 0; tbcv < ${#temp_block_changes_values[@]}; tbcv++)); do

                    #Статус проверки значения. Если проверяемое значение будет соответствовать условию, то переменной будет присвоено значение 1.
                    let status_check_value=0

                    #Если в значении присутствует разделитель :, то продолжаем выполнение
                    if [[ "$(echo "${temp_block_changes_values[$tbcv]}" | grep -o ':' | wc -l)" -eq "1" ]]; then

                      #Получаем значение для сравнения из строки параметра
                      value_for_comparison="$(echo "${temp_block_changes_values[$tbcv]}" | cut -d ':' -f1)"

                      if [[ "$value_for_comparison" == "not_null" ]]; then

                        #Если value_for_comparison равно not_null, то проверяем, что значение в скрипте не пустое
                        if [[ -n "$(sed -nr "{ :l /${listparamchange[i]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" ]] && [[ "$(sed -nr "{ :l /${listparamchange[i]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" != "''" ]] && [[ "$(sed -nr "{ :l /${listparamchange[i]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" != "\"\"" ]] && [[ "$(sed -nr "{ :l /${listparamchange[i]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" != "()" ]]; then
                          check_value_script_and_conf
                        fi
                      else
                        #Иначе сравниваем значение в параметре со значением в скрипте
                        if [[ "$value_for_comparison" == "$(sed -nr "{ :l /${listparamchange[i]}[ ]*=/ { s/[^=]*=[ ]*//; p; q;}; n; b l;}" "$tfs/$tempnamescript")" ]]; then
                          check_value_script_and_conf
                        fi
                      fi

                      if [[ "$status_check_value" -eq "1" ]]; then

                        #Если status_check_value равно 1, то записываем в массив значения (имена секций) из второй части строки (после :) по разделителю ;.
                        readarray -d ';' -t temp_list_block_changes_values < <(echo "${temp_block_changes_values[$tbcv]}" | cut -d ':' -f2 | tr -d '\n')

                        #Перебор значений массива
                        for ((tlbcv = 0; tlbcv < ${#temp_list_block_changes_values[@]}; tlbcv++)); do

                          if [[ "$(printf '%s\n' "${listparamchange[@]}" | grep "^${temp_list_block_changes_values[$tlbcv]}$" | wc -l)" -gt "0" ]]; then

                            #Если секция найдена в script.conf, то проверяем, что значения нет в массиве list_block_changes_values. Если его нет, то добавляем в массив для дальнейшего пропуска вопроса.
                            if [[ "$(printf '%s\n' "${list_block_changes_values[@]}" | grep "^${temp_list_block_changes_values[$tlbcv]}$" | wc -l)" -eq "0" ]]; then
                              list_block_changes_values[${#list_block_changes_values[@]}]="${temp_list_block_changes_values[$tlbcv]}"
                            fi
                          else
                            echo -e "\n${RED}Не найдена секция ${temp_list_block_changes_values[$tlbcv]} в script.conf$NoColor"
                          fi
                        done
                      fi
                    else
                      echo -e "\n${RED}Значение № $tbcv параметра block_changes_values не соответствует формату$NoColor"
                    fi
                  done
                fi
              else
                echo -e "\n${RED}Не заполнено описание переменной ${listparamchange[i]} $NoColor"
              fi
            else
              echo -e "\n${RED}Некорректный тип переменной ${listparamchange[i]} в файле script.conf. Допустимые значения:
number - любое число
text - текстовое поле
...
$NoColor"
            ...
            echo -e "\n${RED}Переменная ${listparamchange[i]} не найдена в файле скрипта$NoColor"
          fi
        else
...
function runsend {
  ...
  if [[ "$num_descriptior" =~ $check_num ]] || [[ "$sshtypecon" = "key" ]]; then
    ...
    if [[ "$multisend" -eq "1" ]]; then
      ...
      #Форматирование log файла (Удаление возврата каретки. Если имеется несколько пустых строк подряд, то оставляем одну.)
      sed -i 's/\r$//' "$dir_logs/$logfile"
      sed -i -e 's/'"$(printf '\015')"'$//' "$dir_logs/$logfile"
      sed -i '$!N;/^\n$/{$q;D;};P;D;' "$dir_logs/$logfile"
    else
      #Каталог лог файлов при многопоточной отправке
      sendlogs="$dir_logs/${value_date}_$(echo "${list_param_send_script[0]}" | cut -d ';' -f 1)_${#list_param_send_script[@]}"
      ...
      sed -i 's/\r$//' "$sendlogs/$namescript"
      sed -i -e 's/'"$(printf '\015')"'$//' "$sendlogs/$namescript"
      sed -i '$!N;/^\n$/{$q;D;};P;D;' "$sendlogs/$namescript"
    fi
            ...
            if [[ "$remdir" -eq "1" ]]; then
              ...
              if [[ -n "$remhost" ]]; then
                ...
                if [[ "$local_version_exec_script" -gt "0" ]]; then
                  ...
                  echo -e "\nВключено сравнение отправляемой и выполненной ранее версии скрипта (если значение 0, то файл версий существует/создан и к нему есть доступ, но строки с версией скрипта в нем еще нет.)\n"
...
function sshrunscript {
  ...
  if [[ "$err_status_exec_script" -eq "0" ]]; then
    ...
    if [[ "$reboot_system_script_finish" -eq "1" ]] && [[ "$num_send_script" -lt "$(expr ${#list_param_send_script[@]} - 1)" ]]; then
      ...
      echo "Ожидание (70 секунд) перезагрузки устройства $remhost (${list_ipall[$i]})."
      sleep 70
