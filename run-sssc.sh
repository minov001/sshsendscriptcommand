...
#Необходимые условия проверки, пока файл функций не подключен
...
#Если файл use-color-text существует, то переход к чтению содержимого, иначе use_color_text равен 1
if [[ -f "$dir_runscript/conf/use-color-text" ]]; then
  #Запись содержимого файла в переменную
  use_color_text="$(cat "$dir_runscript/conf/use-color-text")"

  #Если значение это число и оно равно 0 или 1, то вывод сообщения включено ли использование цветного текста, иначе use_color_text равен 1
  if [[ "$use_color_text" =~ $check_num ]] && [[ "$use_color_text" -eq "0" || "$use_color_text" -eq "1" ]]; then
    echo "Использование цветного текста: $([[ "$use_color_text" -eq "1" ]] && echo "включено" || echo "отключено")"
  else
    echo "Значение в файле ./conf/use-color-text некорректно.
Допустимо использовать 0 или 1.
Использование цветного текста: включено"
    use_color_text="1"
  fi
else
  echo "Файл ./conf/use-color-text не найден.
Использование цветного текста: включено"
  use_color_text="1"
fi
