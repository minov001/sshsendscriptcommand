...
#### Версия stable: 20260903_01 (В соседней ветке dev версии)

#### Рекомендуется ознакомление с инструкцией в docx/pdf файле (В ней есть скриншоты)
...
Список протестированных систем (версия скрипта 20250313_01):
```
1. Astra Linux 1.8.6
2. Альт Рабочая Станция K 11.1
3. RedOS 8
```
...
В каталоге многопоточного логирования создаются следующие файлы:

1.`all-sendinfo.txt` — Содержит общую информацию о выполнении (список успешных/неуспешных выполнений и имя файла потока где смотреть полный лог по устройству)

2.`sssc(0,1,2…)` - Файлы с полным логом потока

3.`log-filter.txt` — Этот файл будет существовать, если вы в скрипте выведите два echo с текстом `#---Начало нужной информации` и `#---Конец нужной информации`. Весь вывод между этими строками будет скопирован в этот файл. В однопоточном режиме в каталоге `logs` будет создан файл с расширением `filter`. Например:

```
#Тест фильтра логов
echo "#---Начало нужной информации ($(hostname)---#)"
echo "Тест 1 $(date +"%Y%m%d-%H%M%S-%4N")"
echo "Тест 2 $(date +"%Y%m%d-%H%M%S-%4N")"
echo "#---Конец нужной информации ($(hostname)---#)"
```
...
- `./conf/use-color-text` - файл настройки цветного текста (0 - отключить, 1 - включить). Если файл не существует или значение некорректно, то используется цветной текст.
- `con_kerberos.sh`, `con_key.sh`, `con_pas.sh` - Файлы с командами подключения ssh, scp, rsync. При необходимости можно добавлять параметры подключения в команды.
...
file_listIgnoreInaccurate=conf/listIgnoreInaccurate
file_listIgnoreAccurate=conf/listIgnoreAccurate
...
- `typesendfiles` - Тип отправки файлов (`scp` или `rsync`. Рекомендуется `rsync`). Если выбран `rsync`, но он не установлен на удаленном устройстве, то автоматически для этого устройства будет использоваться `scp`. `scp` устанавливается с `ssh`.
...
- `sshCheckInterval` - Интервал проверки активности ssh соединения в секундах. Если 0, то ssh будет ждать подключение бесконечно. Значение передается параметру подключения `ServerAliveInterval`. По умолчанию 30.
- `sshCheckCol` - Количество попыток проверки активности ssh соединения, если с соединением возникла проблема. Не рекомендуется ставить значение 1, т.к соединение прервется при первой же проблеме. Если будет значение 0, то по умолчанию будет выставлено 2. Значение будет передано параметру `ServerAliveCountMax`.
- `RsyncTimeout` - Максимальное время ожидания ввода-вывода rsync в секундах. Если в течение указанного времени не будет передано ни одного байта данных, rsync принудительно завершит работу с ошибкой.Если 0, то rsync будет ждать подключение бесконечно. По умолчанию 900.
- `numportssh` - Номер SSH порта (по умолчанию 22, если значение не задано или некорректно). Условие для проверки значения: `'^[0-9]+$'`
- `sshtypecon` - Тип SSH подключения (допустимые значения: `pas` - пароль, `key` - ключ, `kerberos` - билет керберос. Рекомендуется подключение по ключу).
...
- `file_listIgnoreInaccurate` - Путь к файлу (полный путь от корня или относительный путь от каталога запуска скрипта) со списком исключений по частичному совпадению (Необязательная переменная.). В файле должно быть по одному значению на строку. Значения с # в начале строки игнорируются.
- `file_listIgnoreAccurate` - Путь к файлу (полный путь от корня или относительный путь от каталога запуска скрипта) со списком исключений по точному совпадению (Необязательная переменная.). В файле должно быть по одному значению на строку. Значения с # в начале строки игнорируются.
...
- `copy_send_files` - Копировать отправляемые файлы во временный каталог перед отправкой, вместо создания симлинка (0 - создавать симлинк, 1 - копировать. По умолчанию создается симлинк)
...
Если вы поместите в скрипт `exit 1` или последняя выполняемая команда в скрипте присвоит код ошибки, то информация об этом будет в разделе «`Выполнение с ошибкой`» в конце лога.
...
Вопросы задаются в порядке расположения секций в файле. Учитывайте это при использовании `block_changes_values` и `terms_block_changes_curent_variable`.
...
block_changes_values=значение-текущей-переменной1:имя-блокируемой-переменной1;имя-блокируемой-переменной2|значение-текущей-переменной2:имя-блокируемой-переменной1;имя-блокируемой-переменной2
terms_block_changes_curent_variable=проверяемое-значение1:имя-проверяемой-переменной1;имя-проверяемой-переменной2|проверяемое-значение2:имя-проверяемой-переменной1;имя-проверяемой-переменной2
```

Поле `listvalue` обязательно, если выбран тип переменной `list`

`terms_block_changes_curent_variable` - необязательное поле, позволяющее указать условия, при которых вопрос не будет задан для текущей переменной. В проверяемом значении также можно указать not_null для проверки текстового поля, что оно не пустое.

`block_changes_values` - необязательное поле, которое позволяет указать, для каких переменных не задавать вопрос об изменении, если значение текущей переменной соответствует указанному значению/условию. В значении текущей переменной также можно указать not_null для проверки текстового поля, что оно не пустое.
...
[send_notifysend]
typevalue=truefalse
descvalue=Вывод сообщения через notify-send (0 - отключить, 1 - включить)
block_changes_values=0:notifysend_expiretime

[notifysend_expiretime]
typevalue=number
descvalue=Время в миллисекундах, по истечении которого уведомление notify-send исчезнет (По умолчанию 10000 (10 секунд). При указании 0, уведомление автоматически не исчезает)


[send_first_found_dialog]
typevalue=truefalse
descvalue=Вывод сообщения через первое найденное диалоговое окно: yad, fly-dialog, zenity (0 - отключить, 1 - включить)
block_changes_values=1:send_yad;send_flydialog;send_zenity

[send_yad]
typevalue=truefalse
descvalue=Вывод сообщения через yad (0 - отключить, 1 - включить)

[send_flydialog]
typevalue=truefalse
descvalue=Вывод сообщения через fly-dialog (0 - отключить, 1 - включить)

[send_zenity]
typevalue=truefalse
descvalue=Вывод сообщения через zenity (0 - отключить, 1 - включить)

[send_msg_use_root]
typevalue=truefalse
descvalue=Вывод сообщения от имени пользователя или root (0 - от пользователя, 1 - от root)
terms_block_changes_curent_variable=0:send_notifysend;send_first_found_dialog;send_yad;send_flydialog;send_zenity

[typesend]
typevalue=list
descvalue=Тип ввода сообщения: file (необходимо отправить файл с текстом) и text (добавьте текст в массив msgtext)
listvalue=file;text
block_changes_values='file':msgtext
terms_block_changes_curent_variable=0:send_notifysend;send_first_found_dialog;send_yad;send_flydialog;send_zenity

[msgtext]
typevalue=massive
descvalue=Текст сообщения для показа (Если выбран тип ввода сообщения 'text' в typesend)
terms_block_changes_curent_variable=0:send_notifysend;send_first_found_dialog;send_yad;send_flydialog;send_zenity

[headertext]
typevalue=text
descvalue=Заголовок сообщения (Если не заполнен, то используется заданное по умолчанию значение)
terms_block_changes_curent_variable=0:send_notifysend;send_first_found_dialog;send_yad;send_flydialog;send_zenity

[activeusername]
typevalue=massive
descvalue=Добавьте имена пользователей в массив, если необходимо показать сообщение только им (поиск всех активных пользователей будет пропущен)
terms_block_changes_curent_variable=0:send_notifysend;send_first_found_dialog;send_yad;send_flydialog;send_zenity
```
...
- Проверяемые исполняемые файлы: `nmap, sshpass, gpg2, awk, sed, grep, tmux или screen, zenity, ssh, scp или rsync, ls, cut, rev, cat, dialog, curl`.
- Проверяемые пакеты в случае установки: `nmap, sshpass, gnupg2 или gnupg2-gostcrypto, gawk, sed, grep, tmux, screen, zenity, openssh-client или openssh-clients, rsync, dialog, curl`.
...
Примеры команд:
- `./run-sssc.sh -hf "localhost" -sp "sendmessage:sendmessage:autopassudo:0:0`
...
- `./run-sssc.sh -hn "10.0.2.120" -sp "rename-host::autopassudo:1:0" -sp "astra-ad-sssd::autopassudo:1:1" -sp "sss-override-group::autopassudo:0:1" -sp "management-ca-cert:cert:autopassudo:0:1" -sp "cryptopro-install:cprocsp:autopassudo:0:1" -sp "reboot::autopassudo:0:0"`
...
Через 20 секунд после выполнения будет выполнена перезагрузка.
...
- `recovery_ca_cert_firefox` - Восстановить используемую по умолчанию библиотеку корневых сертификтов в firefox (При наличии резервного файла .bak. При его отсутствии нужно переустановить пакет forefox) и удаление файла cron задачи.
...
- `recovery_ca_cert_other` - Восстановить используемую по умолчанию библиотеку корневых сертификтов libnssckbi.so (При наличии резервного файла .bak. При его отсутствии нужно переустановить содержащий файл пакет. Для Debian это libnss3) и удаление файла cron задачи.

3.Добавление корневых сертификатов в системное хранилище.

- `exec_update_ca_cert` - Выполнить обновление системных корневых сертификатов (0 - отключить, 1 — включить).
...
#### reboot
Перезагрузка устройства. Доступны следующие переменные:
- `time_to_reboot` - Время в секундах до перезагрузки (По умолчанию 10 секунд. Выставляйте значение в зависимости от количества/размера переданных файлов, чтобы хватило время на их удаление и корректное отключение ssh сессии).
...
Доступные способы вывода сообщения: `notify-send`, `yad`, `fly-dialog` и `zenity`. По умолчанию все способы показа отключены, необходимо включить нужное в скрипте.

Доступны следующие переменные:

- `send_notifysend` - Вывод сообщения через `notify-send` (0 - отключить, 1 - включить);

- `notifysend_expiretime` - Время в миллисекундах, по истечении которого уведомление `notify-send` исчезнет (По умолчанию 10000 (10 секунд). При указании 0, уведомление автоматически не исчезает);

- `send_first_found_dialog` - Вывод сообщения через первое найденное диалоговое окно: `yad`, `fly-dialog`, `zenity` (0 - отключить, 1 — включить);

- `send_yad` - Вывод сообщения через `yad` (0 - отключить, 1 — включить);

- `send_flydialog` - Вывод сообщения через `fly-dialog` (0 - отключить, 1 — включить);

- `send_zenity` - Вывод сообщения через `zenity` (0 - отключить, 1 — включить);

- `send_msg_use_root` - Вывод сообщения от имени пользователя или root (0 — от пользователя, 1 - от root);

- `typesend` - Тип ввода сообщения: `file` и `text`;

1) `file` — Положите в  каталог (например, `./files/sendmessage`) текстовые файлы с расширением `.smsg` и выберите (или укажите в нужной секции, в случае запуска с параметрами) этот каталог для отправки.

2) `text` — Добавьте необходимый текст в массив msgtext.

- `msgtext` - Текст сообщения для показа (Если выбран тип ввода сообщения `text` в `typesend`);

- `headertext` - Заголовок сообщения (Если не заполнен, то используется заданное по умолчанию значение);

- `activeusername` - Добавьте имена пользователей в массив, если необходимо показать сообщение только им (поиск всех активных пользователей будет пропущен).

Доступен файл `script.conf` для изменения переменных при отправке, без необходимости редактировать оригинальный файл скрипта.

Функция с кодом вывода сообщений вынесена в отдельный файл `./sendfunc/func_send_msg.sh` для возможности подключения и использования в своих скриптах.

Для работы из вашего скрипта без передачи файла с текстом необходимо:
- 1) Инициализировать переменную `typesend="text"`;
- 2) Инициализировать массив `msgtext` с нужным количеством элементов (Каждый элемент массива это отдельное окно сообщения);
- 3) Инициализировать как минимум одну из переменных `send_notifysend`, `send_first_found_dialog`, `send_yad`, `send_flydialog`, `send_zenity` со значением 1;
- 4) При необходимости инициализировать другие переменные из списка выше.
...
- `create_local_group` - Создать локальную группу, если она отсутствует в системе.
Доступен файл `script.conf` для изменения переменных при отправке, без необходимости редактировать оригинальный файл скрипта.
